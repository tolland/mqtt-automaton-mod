...
class PathingClient:
    def __init__(self, settings: Settings, waypoints: List[Dict[str, Any]], patterns: Dict[str, List[str]]):
        self.s = settings
        ...
        self._waypoint_processing_paused = False  # Flag to pause waypoint processing during events
    ...
    def _is_arrival(data: Dict[str, Any]) -> bool:
        ...
        if data.get("status") == "success" and "x" in data and "y" in data and "z" in data:
            return True
    ...
    def _resolve_step(self, step: str, base_xyz: Tuple[float,float,float]) -> Tuple[int,int,int]:
        tok = step.split()
        if len(tok) != 3:
        ...
        for i, wp in enumerate(self.waypoints, start=1):
            ...
            while not self._attempt_until_arrival(go_cmd):
                if self._waypoint_processing_paused:
                    print(f"[waypoint] Waiting for event handler to complete before continuing...")
            ...
            for pname in run_list:
                ...
                for sidx, step in enumerate(steps, start=1):
                    ...
                    if self._waypoint_processing_paused:
                        print(f"[waypoint] Pattern step {sidx} paused - waiting for event handler to complete...")
                        while self._waypoint_processing_paused:
                            time.sleep(0.5)
                    ...
                    if not isinstance(step, str):
                        print(f"[error] Pattern step {sidx} must be string coordinates or dwell dict, got: {type(step)}")
                        return 2
                    ...
                    while not self._attempt_until_arrival(cmd):
                        if self._waypoint_processing_paused:
                            print(f"[waypoint] Waiting for event handler to complete before continuing pattern step...")
                            while self._waypoint_processing_paused:
                                time.sleep(0.5)
                            print(f"[waypoint] Event handler completed, retrying pattern step...")
                        else:
                            return 2  # Actual failure, not pause
    ...
    def _attempt_until_arrival(self, cmd: str) -> bool:
        ...
        if self._waypoint_processing_paused:
            print(f"[waypoint] Skipping command - waypoint processing paused for event handler")
            return False
    ...
    def _start_event_sequence(self, event_type: str, steps: List[Dict[str, Any]]):
        ...
        is_lifecycle_event = event_type in ["waypoints_start", "waypoint_start", "waypoint_end", "waypoints_end", "pattern_start", "pattern_end"]
        ...
        event_thread = threading.Thread(
            target=self._execute_event_sequence,
            args=(event_type,),
            daemon=True
        )
        event_thread.start()
            ...
            with self._lock:
                ...
                if self._last_event:
                    ...
                    if event_request_id == current_request_id:
                        ...
                        if self._last_event.get("status") == "success" and "x" in self._last_event and "y" in self._last_event and "z" in self._last_event:
...
def build_settings(args, cfg: Dict[str, Any]) -> Settings:
    ...
    if not client_id:
        print("Error: --client-id or YAML client_id required", file=sys.stderr); sys.exit(2)
...
def main():
    ap = argparse.ArgumentParser(description="Baritone pathing client with patterns (MQTT + YAML)")
    ...
    if not waypoints:
        print("Error: YAML missing 'waypoints'", file=sys.stderr); sys.exit(2)
