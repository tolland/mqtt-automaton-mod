...
class PatternEngine:
    ...
    def __init__(self, message_sender: Callable[[str], None], 
    ...
    def resolve_coordinates(self, step: str, base_xyz: Tuple[float, float, float]) -> Tuple[int, int, int]:
        """
    ...
    def execute_pattern(self, pattern_name: str, pattern_steps: List[Union[str, Dict[str, Any]]], 
                       start_position: Tuple[float, float, float] = None) -> bool:
        """
        ...
        print(f"[pattern] Executing pattern '{pattern_name}' with {len(pattern_steps)} steps")
            ...
            if isinstance(step, dict):
                ...
                try:
                    ...
                    if not self.execute_movement(target_coords):
                        ...
                        return False
                    current_pos = (float(target_coords[0]), float(target_coords[1]), float(target_coords[2]))
