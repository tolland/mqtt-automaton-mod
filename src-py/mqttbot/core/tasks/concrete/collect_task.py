...
class CollectTask(TaskBase):
    ...
    def __init__(
        ...
        self.range_ = params.get("range", 5)
        self.result = None
        self._timeout = 60.0
        self._sent_time = 0.0
    ...
    def _step(self, ctx: Context) -> TaskStatus:
        ...
        if self._internal_status == TaskInternalState.READY:
            # Send request
            logger.info(f"[CollectTask] Sending collect {self.origin}")
            ...
            message_data = ServiceMessage(
                ...
                params={"block": self.block, "origin": self.origin, "range": self.range_},
            )
            ...
            # Check timeout
                ...
                logger.info(f"[CollectTask] Timeout waiting for goto {self.origin}")
            ...
            if self.result.status == RequestStatus.SUCCESS:
                logger.debug(f"Reached {self.origin}")
                self.transition_to(TaskInternalState.DONE)
            else:
                logger.warning(f"Failed to reach {self.origin}: {self.result.error}")
            ...
            raise NotImplementedError(f"CollectTask: Unhandled internal state {self._internal_status}")
    ...
    def handle_cancel(self, ctx: Context) -> None:
        ...
        if not self.request_id:
            raise ValueError("Cannot cancel CollectTask without a valid request_id")

        logger.info(f"[CollectTask] origin: {self.origin}, cancelling request {self.request_id}")
