...
public class MqttBotConfig {
    private static final Logger LOGGER = LoggerFactory.getLogger("mqttbot-config");
    ...
    private void loadConfig() {
        try {
            ...
            if (!Files.exists(configPath)) {
                LOGGER.info("Config file not found at {}, using defaults", configPath.toAbsolutePath());
                return;
            }
            ...
            try (InputStream inputStream = new FileInputStream(configPath.toFile())) {
                ...
                if (data != null) {
                    ...
                    // Load bot config
                    @SuppressWarnings("unchecked")
                    Map<String, Object> botConfig = (Map<String, Object>) data.get("bot");
                    if (botConfig != null) {
                        if (botConfig.containsKey("debug_events")) {
                            debugEvents = (Boolean) botConfig.get("debug_events");
                        }
