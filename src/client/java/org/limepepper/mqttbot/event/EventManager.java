...
public final class EventManager {
    private static final MqttBotLogger LOGGER = new MqttBotLogger(EventManager.class);
    ...
    private <L extends Listener, E extends Event<L>> void fireImpl(E event) {
        ...
        try {
            ...
            LOGGER.debugEvents("EventManager.fireImpl - Event type: {}", type.getName());
            LOGGER.debugEvents("Listeners found: {}", (listeners != null ? listeners.size() : "null"));
            if (listeners != null) {
                for (int i = 0; i < listeners.size(); i++) {
                    L listener = listeners.get(i);
                    if (listener != null) {
                        LOGGER.debugEvents("Listener {}: {}", i, listener.getClass().getName());
                    } else {
            ...
            for (L listener : listeners2) {
                if (listener == null) nullCount++;
            }
            if (nullCount > 0) {
                LOGGER.warn("Found {} null listeners, filtering them out", nullCount);
            }
            
            listeners2.removeIf(Objects::isNull);
            
            LOGGER.debugEvents("After filtering nulls, listeners count: {}", listeners2.size());
            if (!listeners2.isEmpty()) {
                LOGGER.debugEvents("About to fire event to {} listeners", listeners2.size());
                event.fire(listeners2);
                LOGGER.debugEvents("Event fired successfully");
            } else {
                LOGGER.debugEvents("No valid listeners remaining after null filtering!");
            }
