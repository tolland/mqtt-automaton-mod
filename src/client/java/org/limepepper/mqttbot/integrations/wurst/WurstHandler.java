...
public class WurstHandler extends Action implements MqttMessageListener {
    ...
    private static class CommandTask {
        ...
        CommandTask(MessageData originalData, String commandName, List<String> args) {
    ...
    public static void init() {
        EventManager.INSTANCE.add(MqttMessageListener.class, new WurstHandler());
    ...
    @Override
    public void onMessageArrived(MqttMessageListener.MqttMessageEvent mqttMessageEvent) {
        ...
        switch (data.getMethod()) {
            case "command":
                ...
                break;
            default:
                sendErrorReply(data, "Unknown method: " + data.getMethod(), "UNKNOWN_METHOD");
        }
    }
    ...
    private void handleArbitraryCommand(MessageData data) {
        try {
            ...
            if (params == null) {
                sendErrorReply(data, "No parameters provided for command", "MISSING_PARAMETERS");
                return;
            }
            ...
            if (params.has("command")) {
                ...
                sendErrorReply(data, "No 'command' parameter specified", "MISSING_COMMAND");
                return;
            }
            ...
            if (params.has("args")) {
                if (params.get("args").isJsonArray()) {
                    ...
                    params.get("args").getAsJsonArray().forEach(element -> 
            ...
            sendErrorReply(data, "Error parsing command parameters: " + e.getMessage(), "PARSE_ERROR");
        }
    }
    
    /**
     * Execute a Wurst command on the main client thread (called from tick event)
     */
    private static void executeWurstCommandOnMainThread(CommandTask task) {
        try {
            ...
            if (cmd == null) {
                sendErrorReplyStatic(task.originalData, "Command not found: " + task.commandName, "COMMAND_NOT_FOUND");
                return;
            }
            
            System.out.println("Executing Wurst command: " + task.commandName + " with args: " + task.args);
            cmd.call(task.args.toArray(new String[0]));
            
            // Send success reply
            sendSuccessReplyStatic(task.originalData, "Command executed successfully: " + task.commandName + " " + String.join(" ", task.args));
            
        } catch (CmdException e) {
            String errorMsg = "Wurst command failed: " + e.getMessage();
            System.err.println(errorMsg);
            sendErrorReplyStatic(task.originalData, errorMsg, "COMMAND_EXECUTION_ERROR");
        } catch (Exception e) {
            String errorMsg = "Unexpected error executing Wurst command: " + e.getMessage();
            System.err.println(errorMsg);
            sendErrorReplyStatic(task.originalData, errorMsg, "UNEXPECTED_ERROR");
        }
    }
     ...
     */
    private void sendErrorReply(MessageData originalData, String errorMessage, String errorCode) {
    ...
    private static void sendErrorReplyStatic(MessageData originalData, String errorMessage, String errorCode) {
        try {
            ...
            if (originalData.getMethod() != null) {
                errorParams.addProperty("originalMethod", originalData.getMethod());
            }
            
            MessageData replyData = new MessageData(
                "wurst",
                "response", 
                originalData.getRequestId(),
            ...
            EventManager.fire(new MqttReplyListener.MqttReplyEvent(botId, replyData));
    ...
    private static void sendSuccessReplyStatic(MessageData originalData, String successMessage) {
        try {
            ...
            if (originalData.getMethod() != null) {
                successParams.addProperty("originalMethod", originalData.getMethod());
            }
            ...
            System.err.println("Failed to send success reply: " + e.getMessage());
        }
    }
}
