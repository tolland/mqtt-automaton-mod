...
public class MqttClientInternal extends Action implements MqttReplyListener {
    private static final MqttBotLogger LOGGER = new MqttBotLogger(MqttClientInternal.class);
        ...
        try {
            ...
            sampleClient.setCallback(new MqttCallback() {
                ...
                public void messageArrived(String topic, MqttMessage message) throws Exception {
                    LOGGER.debugMqtt("MQTT message received on topic {}: {}", topic, message.toString());
            ...
            LOGGER.info("Subscribing to MQTT topics: mqttbot/bots/command, {}", clientTopic);
            sampleClient.subscribe(clientTopic);

            LOGGER.debugMqtt("Registering MqttClientInternal as MqttReplyListener");
     ...
     * @param rawMessage The raw message content (expected to be JSON)
     */
    public void handleMqttMessage(String topic, String rawMessage) {
        LOGGER.debugMqtt("Handling MQTT message on topic: {}", topic);
        if (!topic.startsWith("mqttbot")) return;
        if (!topic.endsWith("command")) return;
        String botId = topic.split("/")[1]; // Extracts 'Player123' from 'mqttbot/Player123/command'
        ...
        if (structuredMessage != null && structuredMessage.isValid()) {
            LOGGER.debug("Firing MqttMessageEvent for valid structured message");
            EventManager.fire(new MqttMessageListener.MqttMessageEvent(botId, structuredMessage));
        } else {
            // Fallback: try to handle as legacy format based on topic
            LOGGER.warn("Received invalid/legacy message format: {}", rawMessage);
        }
    }
    ...
    public void publish(String topic, String message) {
        ...
        try {
            ...
            LOGGER.error("Failed to publish MQTT message to topic {}: {} (reason: {}, cause: {})", 
    ...
    public void onReplyArrived(MqttReplyEvent mqttReplyEvent) {
        LOGGER.debug("Handling MqttReplyEvent for bot: {}", mqttReplyEvent.botId);
        LOGGER.debugMqtt("Reply data: {}", mqttReplyEvent.messageData.toJson());
        publish("mqttbot/" + mqttReplyEvent.botId + "/reply", mqttReplyEvent.messageData.toJson());
    }
}
