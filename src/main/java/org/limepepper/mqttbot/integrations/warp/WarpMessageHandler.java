...
public final class WarpMessageHandler extends Action
    ...
    public void onMessageArrived(MqttMessageEvent mqttMessageEvent)
    {
        ...
        switch(data.getMethod())
        {
            ...
            default:
            System.out
                .println("Unknown method in warp service: " + data.getMethod());
        }
    }
    ...
    private void handleTeleportCommand(MessageData data)
    {
        try
        {
            ...
            // Extract parameters
            String warpName =
                params.has("name") ? params.get("name").getAsString() : null;
            Integer radius =
                params.has("radius") ? params.get("radius").getAsInt() : 5;
            String commandTemplate = params.has("command_template")
                ? params.get("command_template").getAsString() : "warp {name}";
            ...
            // Build warp command using template from params (supports different
            // server plugins)
            ...
            WarpUtil.start(data.getRequestId(), warpName, targetX, targetY,
                targetZ, radius);
