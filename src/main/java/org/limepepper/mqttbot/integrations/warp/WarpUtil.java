...
public final class WarpUtil {
     ...
     * @param name
     *            Warp destination name
     * @param targetX
     *            Expected destination X coordinate
     * @param targetY
     *            Expected destination Y coordinate
     * @param targetZ
     *            Expected destination Z coordinate
     * @param radiusOverride
     *            Optional radius override (default 5 blocks)
     */
    public static void start(String reqId, String name, double targetX,
        double targetY, double targetZ, Integer radiusOverride)
    {
        enabled = true;
        requestId = reqId;
        warpName = name;
        ...
        radius =
            (radiusOverride != null && radiusOverride > 0) ? radiusOverride : 5;
    ...
    {
        if(!enabled || phase != Phase.WAITING_FOR_TELEPORT
            || client.player == null)
        {
            return;
        }
        ...
        {
            fail("Warp timeout - took longer than " + (MAX_WAIT_TICKS / 20)
                + " seconds");
            return;
        }
        ...
        {
            System.out
                .println("[WarpUtil] Waiting for warp... Distance to target: "
        ...
        String reason)
    {
        ...
        {
            System.err
                .println("Error sending warp response: " + e.getMessage());
            e.printStackTrace();
        }
    }
}
