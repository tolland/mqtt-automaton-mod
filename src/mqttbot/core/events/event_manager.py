    ...
    def _load_config(self) -> None:
        ...
        for service_name, methods in self.config.items():
            for method_name, handler_config in methods.items():
                ...
                self.handlers[key] = EventHandlerConfig(
                    enabled=True,
                    steps=steps
                )
                print(f"[config] Registered event handler: {service_name}:{method_name} ({len(steps)} steps)")
