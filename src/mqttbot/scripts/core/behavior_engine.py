...
class BehaviorEngine:
    ...
    def queue_behavior(self, behavior: BaseBehavior) -> None:
        ...
        with self.lock:
            ...
            print(
                f"[behavior_engine] Queued behavior: {behavior.name} (priority: {behavior.priority})"
            )
        ...
        while self.running:
            try:
                with self.lock:
                    ...
                    if self.active_behavior and self._should_interrupt_current(
                        next_behavior
                    ):
                    ...
                    if self.active_behavior and self.active_behavior.state in [
    ...
    def _get_next_behavior(self) -> Optional[BaseBehavior]:
        ...
        if self.emergency_mode:
            for behavior in self.behaviors:
                if (
                    behavior.name == "emergency"
                    and behavior.state == BehaviorState.IDLE
                ):
                    return behavior
            return None
        ...
        for behavior in self.behavior_queue:
            if behavior.state == BehaviorState.IDLE and behavior.can_start(
                self.context
            ):
                return behavior
    ...
    def _start_behavior(self, behavior: BaseBehavior) -> None:
        ...
        if behavior.start(self.context):
            ...
            behavior_thread = threading.Thread(
                target=self._execute_behavior, args=(behavior,), daemon=True
            )
    ...
    def _execute_behavior(self, behavior: BaseBehavior) -> None:
        ...
        try:
            ...
            if success:
                print(
                    f"[behavior_engine] Behavior completed successfully: {behavior.name}"
                )
            else:
